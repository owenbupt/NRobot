...
function agent_parameters = load_agent_parameters( filename )
	...
	switch agent_parameters.dynamics
		case 0 %DYNAMICS_SI_GROUND_XY
			agent_parameters.states = {'x','y'};
			agent_parameters.number_of_states = length(agent_parameters.states);
			agent_parameters.number_of_inputs = 2;
			agent_parameters.control_input_gains = fscanf(f, '%f', ...
			  agent_parameters.number_of_inputs);
			
		case 1 % DYNAMICS_SI_GROUND_XYy
			agent_parameters.states = {'x','y','yaw'};
			agent_parameters.number_of_states = length(agent_parameters.states);
			agent_parameters.number_of_inputs = 3;
			agent_parameters.control_input_gains = fscanf(f, '%f', ...
			  agent_parameters.number_of_inputs);
			
		case 2 % DYNAMICS_SI_AIR_XYZ
			agent_parameters.states = {'x','y','z'};
			agent_parameters.number_of_states = length(agent_parameters.states);
			agent_parameters.number_of_inputs = 3;
			agent_parameters.control_input_gains = fscanf(f, '%f', ...
			  agent_parameters.number_of_inputs);
		case 3 % DYNAMICS_SI_AIR_XYZy
			agent_parameters.states = {'x','y','z','yaw'};
			agent_parameters.number_of_states = length(agent_parameters.states);
			agent_parameters.number_of_inputs = 4;
			agent_parameters.control_input_gains = fscanf(f, '%f', ...
			  agent_parameters.number_of_inputs);
		case 4 % DYNAMICS_DUBINS_GROUND_XYy
			agent_parameters.states = {'x','y','yaw'};
			agent_parameters.number_of_states = length(agent_parameters.states);
			agent_parameters.number_of_inputs = 2;
			agent_parameters.control_input_gains = fscanf(f, '%f', ...
	...
	% sensing region
	if ~isempty(agent_parameters.base_relaxed_sensing)
		first_contour_end = ...
...
% containing it.
function agent_state = load_agent_state( filename, number_of_agents, number_of_inputs )
	...
	agent_state.connected = d(:,15:15+number_of_agents-1);
	agent_state.inputs = ...
	  d(:,15+number_of_agents:15+number_of_agents+number_of_inputs-1);
end
